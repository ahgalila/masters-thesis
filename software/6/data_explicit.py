...
class DataLoader(object):
    ...
    def getData(self, cross_validation_index, num_validation, num_test, num_with_symbols):
        ...
        for index, sample in enumerate(train):
            ...
            if sample["hasSymbol"]:
                step_operand_1 = sample["right-class"] + [0.5 for i in range(40)]
                step_operand_2 = sample["right-class"] + sample["left-class"] + [0.5 for i in range(30)]
                step_operator = sample["right-class"] + sample["left-class"] + sample["operator-class"] + [0.5 for i in range(20)]
                step_equals = sample["right-class"] + sample["left-class"] + sample["operator-class"] + sample["target"]
            else:
                step_operand_1 = [0.5 for i in range(50)]
                step_operand_2 = [0.5 for i in range(50)]
                step_operator = [0.5 for i in range(50)]
                step_equals = [0.5 for i in range(30)] + sample["target"]
            train_y.append([step_operand_1, step_operand_2, step_operator, step_equals])
        ...
        for index, sample in enumerate(validation):
            ...
            step_operator = sample["right-class"] + sample["left-class"] + sample["operator-class"] + [0.5 for i in range(20)]
            step_equals = sample["right-class"] + sample["left-class"] + sample["operator-class"] + sample["target"]
            val_y.append([step_operand_1, step_operand_2, step_operator, step_equals])
        ...
        for sample in test:
            ...
            step_operator = sample["right-class"] + sample["left-class"] + sample["operator-class"] + [0.5 for i in range(20)]
            step_equals = sample["right-class"] + sample["left-class"] + sample["operator-class"] + sample["target"]
            test_y.append([step_operand_1, step_operand_2, step_operator, step_equals])
