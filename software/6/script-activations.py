...
from utils import argmax, renderImage, renderDigit, getOperator, renderActivations, renderActivationsBig, renderActivations200
from PIL import Image, ImageDraw, ImageFont

def renderResults(name, results, test_x, activations_0, activations_1):
    
    image = Image.new('RGB', (28 * 3 + 512, len(results) * 180))
    draw = ImageDraw.Draw(image)

    for index, result in enumerate(results):

        renderDigit(draw, test_x[index][0], 0 * 28, index * 180 + 0 * 36)
        renderImage(image, "MNIST_data/" + str(argmax(result[0][10:])) + ".png", 2 * 28, index * 180 + 0 * 36)
        renderActivations(draw, activations_0[index][0], 3 * 28, index * 180 + 0 * 36)
        renderActivations(draw, activations_1[index][0], 3 * 28, index * 180 + 0 * 36 + 16)

        renderDigit(draw, test_x[index][1], 0 * 28, index * 180 + 1 * 36)
        renderImage(image, "MNIST_data/" + str(argmax(result[1][10:])) + ".png", 2 * 28, index * 180 + 1 * 36)
        renderActivations(draw, activations_0[index][1], 3 * 28, index * 180 + 1 * 36)
        renderActivations(draw, activations_1[index][1], 3 * 28, index * 180 + 1 * 36 + 16)

        renderDigit(draw, test_x[index][2], 0 * 28, index * 180 + 2 * 36)
        renderImage(image, "MNIST_data/" + getOperator(result[2][:4]) + ".png", 2 * 28, index * 180 + 2 * 36)
        renderActivations(draw, activations_0[index][2], 3 * 28, index * 180 + 2 * 36)
        renderActivations(draw, activations_1[index][2], 3 * 28, index * 180 + 2 * 36 + 16)

        renderDigit(draw, test_x[index][3], 0 * 28, index * 180 + 3 * 36)
        renderImage(image, "MNIST_data/" + str(argmax(result[3][:10])) + ".png", 1 * 28, index * 180 + 3 * 36)
        renderImage(image, "MNIST_data/" + str(argmax(result[3][10:20])) + ".png", 2 * 28, index * 180 + 3 * 36)
        renderActivations(draw, activations_0[index][3], 3 * 28, index * 180 + 3 * 36)
        renderActivations(draw, activations_1[index][3], 3 * 28, index * 180 + 3 * 36 + 16)
...
def renderResults200(name, results, test_x, activations_0, activations_1):
    ...
    for index, result in enumerate(results):
        ...
        renderDigit(draw, test_x[index][0], 0 * 28, index * 180 + 0 * 36)
        renderImage(image, "MNIST_data/" + str(argmax(result[0][10:])) + ".png", 2 * 28, index * 180 + 0 * 36)
        renderActivations200(draw, activations_0[index][0], 3 * 28, index * 180 + 0 * 36)
        renderActivations200(draw, activations_1[index][0], 3 * 28, index * 180 + 0 * 36 + 16)

        renderDigit(draw, test_x[index][1], 0 * 28, index * 180 + 1 * 36)
        renderImage(image, "MNIST_data/" + str(argmax(result[1][10:])) + ".png", 2 * 28, index * 180 + 1 * 36)
        renderActivations200(draw, activations_0[index][1], 3 * 28, index * 180 + 1 * 36)
        renderActivations200(draw, activations_1[index][1], 3 * 28, index * 180 + 1 * 36 + 16)

        renderDigit(draw, test_x[index][2], 0 * 28, index * 180 + 2 * 36)
        renderImage(image, "MNIST_data/" + getOperator(result[2][:4]) + ".png", 2 * 28, index * 180 + 2 * 36)
        renderActivations200(draw, activations_0[index][2], 3 * 28, index * 180 + 2 * 36)
        renderActivations200(draw, activations_1[index][2], 3 * 28, index * 180 + 2 * 36 + 16)

        renderDigit(draw, test_x[index][3], 0 * 28, index * 180 + 3 * 36)
        renderImage(image, "MNIST_data/" + str(argmax(result[3][:10])) + ".png", 1 * 28, index * 180 + 3 * 36)
        renderImage(image, "MNIST_data/" + str(argmax(result[3][10:20])) + ".png", 2 * 28, index * 180 + 3 * 36)
        renderActivations200(draw, activations_0[index][3], 3 * 28, index * 180 + 3 * 36)
        renderActivations200(draw, activations_1[index][3], 3 * 28, index * 180 + 3 * 36 + 16)

    image.save(name  + ".png")

def renderResultsBig(name, results, test_x, activations):
    ...
    for index, result in enumerate(results):
        ...
        renderDigit(draw, test_x[index][0], 0 * 28, index * 160 + 0 * 32)
        renderImage(image, "MNIST_data/" + str(argmax(result[0][10:])) + ".png", 2 * 28, index * 160 + 0 * 32)
        renderActivationsBig(draw, activations[index][0], 3 * 28, index * 160 + 0 * 32)

        renderDigit(draw, test_x[index][1], 0 * 28, index * 160 + 1 * 32)
        renderImage(image, "MNIST_data/" + str(argmax(result[1][10:])) + ".png", 2 * 28, index * 160 + 1 * 32)
        renderActivationsBig(draw, activations[index][1], 3 * 28, index * 160 + 1 * 32)

        renderDigit(draw, test_x[index][2], 0 * 28, index * 160 + 2 * 32)
        renderImage(image, "MNIST_data/" + getOperator(result[2][:4]) + ".png", 2 * 28, index * 160 + 2 * 32)
        renderActivationsBig(draw, activations[index][2], 3 * 28, index * 160 + 2 * 32)

        renderDigit(draw, test_x[index][3], 0 * 28, index * 160 + 3 * 32)
        renderImage(image, "MNIST_data/" + str(argmax(result[3][:10])) + ".png", 1 * 28, index * 160 + 3 * 32)
        renderImage(image, "MNIST_data/" + str(argmax(result[3][10:20])) + ".png", 2 * 28, index * 160 + 3 * 32)
        renderActivationsBig(draw, activations[index][3], 3 * 28, index * 160 + 3 * 32)
...
while k_fold_index < k:
    ...
    model = LSTMModel(4, [200, 200], 784, 20)
    ...
    for index, result in enumerate(results):
        ...
        operator = argmax(test_y[index][2][-20:-10])
        if operator == 0:
...
#plus
[test_x, test_y] = testDataLoader.getData("+", 0, 6, 0, 6)
results = bestModel.predict(test_x)
activations_0 = bestModel.getActivations(0, test_x)
activations_1 = bestModel.getActivations(1, test_x)
renderResults200("activation-results-200-1-0", results, test_x, activations_0, activations_1)

[test_x, test_y] = testDataLoader.getData("+", 0, 6, 6, 10)
results = bestModel.predict(test_x)
activations_0 = bestModel.getActivations(0, test_x)
activations_1 = bestModel.getActivations(1, test_x)
renderResults200("activation-results-200-2-0", results, test_x, activations_0, activations_1)

[test_x, test_y] = testDataLoader.getData("+", 6, 10, 0, 6)
results = bestModel.predict(test_x)
activations_0 = bestModel.getActivations(0, test_x)
activations_1 = bestModel.getActivations(1, test_x)
renderResults200("activation-results-200-3-0", results, test_x, activations_0, activations_1)

[test_x, test_y] = testDataLoader.getData("+", 6, 10, 6, 10)
results = bestModel.predict(test_x)
activations_0 = bestModel.getActivations(0, test_x)
activations_1 = bestModel.getActivations(1, test_x)
renderResults200("activation-results-200-4-0", results, test_x, activations_0, activations_1)

#plus
#[test_x, test_y] = testDataLoader.getData("+", 0, 6, 0, 6)
#results = bestModel.predict(test_x)
#activations = bestModel.getActivations(1, test_x)
#renderResultsBig("activation-results-big-1-0", results, test_x, activations)

#[test_x, test_y] = testDataLoader.getData("+", 0, 6, 6, 10)
#results = bestModel.predict(test_x)
#activations = bestModel.getActivations(1, test_x)
#renderResultsBig("activation-results-big-2-0", results, test_x, activations)

#[test_x, test_y] = testDataLoader.getData("+", 6, 10, 0, 6)
#results = bestModel.predict(test_x)
#activations = bestModel.getActivations(1, test_x)
#renderResultsBig("activation-results-big-3-0", results, test_x, activations)

#[test_x, test_y] = testDataLoader.getData("+", 6, 10, 6, 10)
#results = bestModel.predict(test_x)
#activations = bestModel.getActivations(1, test_x)
#renderResultsBig("activation-results-big-4-0", results, test_x, activations)
